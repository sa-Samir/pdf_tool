...
| 2 | **App size. Measured 2026-09-23:** the host engine is **22 MB** at full capability and **13 MB** with `keep: auto` trimming it to the capabilities this app calls (core, render, extract) -- a 41% saving, now configured in pubspec.yaml. A debug iOS simulator build carried a 46 MB framework before trimming; a release device build still needs measuring. | Medium | Use the package's `keep:` tree-shaking to include only the capabilities in scope; Android App Bundle splits per ABI; measure download size in week 2, not week 20; budget in requirements §13. |
